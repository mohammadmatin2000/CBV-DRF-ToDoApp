...
class UserManager(BaseUserManager):
    ...
    def create_user(self, email, password, **extra_fields):
        ...
        email = self.normalize_email(
            email
        )  # Normalizes email format (e.g., lowercase handling)
        user = self.model(
    ...
    def create_superuser(self, email, password, **extra_fields):
        ...
        extra_fields.setdefault(
            "is_staff", True
        )  # Ensures superuser has staff access
        extra_fields.setdefault(
            "is_superuser", True
        )  # Ensures superuser privileges
        extra_fields.setdefault(
            "is_active", True
        )  # Activates the account by default
        extra_fields.setdefault("is_verified", True)

        if extra_fields.get("is_staff") is not True:
            raise ValueError(
                "Superuser must have is_staff=True."
            )  # Ensures proper staff status
        ...
        )  # Calls the base user creation method
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    )  # Defines email as the unique identifier
    is_staff = models.BooleanField(
        default=True
    )  # Determines whether the user can access the admin panel
    ...
    objects = (
        UserManager()
    )  # Assigns the custom user manager to handle user creation
