...
from .serializer import (CustomRegistrationApiView, CustomLoginApiView, CustomProfileSerializer,
                         CustomChangePasswordSerializer, CustomActivationResendSerializer,
...
class RegisterApiView(generics.GenericAPIView):
    ...
    permission_classes = [permissions.AllowAny]  # Allows any user (authenticated or not) to register

    def post(self, request, *args, **kwargs):
        """
        Handles user registration via POST request.
        """
        serializer = self.get_serializer(data=request.data)  # Serializes the request data
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(users, many=True)  # Serializes multiple user instances
...
class CustomAuthToken(ObtainAuthToken):
    ...
    def post(self, request, *args, **kwargs):
        ...
        token, created = Token.objects.get_or_create(user=user)  # Generates or retrieves an authentication token
        return Response({
            'token': token.key,
            'user_id': user.pk,
            'email': user.email
...
class CustomProfileView(generics.GenericAPIView):
    ...
    permission_classes = [permissions.IsAuthenticated]  # Ensures only authenticated users can access profile data
    ...
    def get_object(self):
        ...
        obj = get_object_or_404(queryset, user=self.request.user)  # Ensures the profile exists
        return obj
# ======================================================================================================================
...
class CustomChangePasswordView(generics.GenericAPIView):
    ...
    permission_classes = [permissions.IsAuthenticated]  # Ensures only logged-in users can change passwords
    queryset = User.objects.all()  # Retrieves all users
    ...
    def put(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            user.set_password(serializer.data.get("new_password"))  # Securely updates the password
            user.save()
...
class CustomResetPasswordView(generics.GenericAPIView):
    ...
    permission_classes = [permissions.IsAuthenticated]  # Ensures only authenticated users can reset passwords
