...
class TaskViewSet(ModelViewSet):
    ...
    queryset = ToDoApp.objects.all()  # Retrieves all task objects from the database
    ...
    filterset_fields = {
        "id": ["exact"],  # Enables filtering tasks by an exact ID match
        "content": ["exact"],  # Allows exact text matching for task content
        "author": ["exact", "in"],  # Supports filtering by an exact author or multiple authors
    }

    pagination_class = CustomPagination  # Specifies a custom pagination class for managing paginated responses

    search_fields = ['author', 'content']  # Enables search functionality across author and content fields

    ordering_fields = ['id']  # Allows tasks to be ordered based on their ID
